                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  v_dot = (R.col(2) * thrust              // 旋翼推力（沿机体Z轴方向）
        - mass_ * g_ * Eigen::Vector3d::UnitZ() // 重力（全局Z轴负方向）
        + external_force_               // 外部施加的力（如风扰）
        - resistance * vnorm)           // 空气阻力（与速度方向相反）
        / mass_;                        // 牛顿第二定律 F=ma
  ...
  omega_dot = J_.inverse() * (
    moments                     // 电机产生的力矩
    + external_moment_          // 外部施加的力矩
    - cur_state.omega.cross(J_ * cur_state.omega) // 科里奥利力项（刚体旋转耦合项）
  );
